...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        ...
        // Add prefered vote to the candidate's total
...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // If more than half the votes, candidate is the winner.
...
int find_min(void)
{
    // Initialize int to voter count, for when one candidate gets all the votes.
    int lowest_vote = voter_count;

    // Iterate over candidates
    for (int i = 0; i < candidate_count; i++)
    {
        // Check if vote count is less than lowest value thus far.
        if ((candidates[i].votes < lowest_vote) && !candidates[i].eliminated)
        {
            // If lower: update lowest value.
...
void eliminate(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // If number of votes is equal to the minimal number of votes, eliminate the candidate.
        if (candidates[i].votes == min)
        {
